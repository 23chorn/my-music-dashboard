...
export default function DatabaseHealth({ data = {} }) {
  ...
  return (
    <div className="bg-surface-900 rounded-lg p-6">
      <h2 className="text-2xl font-bold text-white mb-6">Database Health</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="text-center">
          ...
          <div className="text-surface-400 text-sm">Total Records</div>
        </div>
        <div className="text-center">
          <div className="text-3xl font-bold text-white mb-1">
            {formatBytes(databaseSize)}
          </div>
          <div className="text-surface-400 text-sm">Database Size</div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center py-2 border-b border-surface-800">
          <span className="text-surface-400">Index Health</span>
          <span className="text-white font-medium">{indexHealth}%</span>
        </div>

        {orphanedRecords > 0 && (
          <div className="flex justify-between items-center py-2 border-b border-surface-800">
            <span className="text-surface-400">Orphaned Records</span>
            <span className="text-danger-400 font-medium">{formatValue(orphanedRecords)}</span>
          </div>
        )}
      </div>
          ...
          <div className="space-y-2">
            {Object.entries(tableStats).map(([table, count]) => (
              <div key={table} className="flex justify-between items-center text-sm">
                <span className="text-surface-400 capitalize">{table}</span>
