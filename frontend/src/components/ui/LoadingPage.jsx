...
export default function LoadingPage({ message = null, showMusicMessages = true }) {
  ...
  return (
    <div className="fixed inset-0 bg-surface-950 flex flex-col items-center justify-center space-y-6 z-50">
      <div className="flex flex-col items-center space-y-6">
        <LoadingSpinner size="xl" className="scale-150" />
        <div 
          className={`text-xl text-surface-300 font-medium text-center transition-opacity duration-300 ${fadeClass} px-4`}
        >
          {displayMessage}
        </div>
      </div>
