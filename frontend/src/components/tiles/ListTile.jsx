...
export default function ListTile({ label, value, sub, link, album, rank, image }) {
  ...
  const mainContent = (
    <span className="flex items-center gap-3 min-w-0 w-full">
      ...
      <span className="flex flex-col min-w-0 flex-1">
        ...
        {/* Mobile only: shown indented under the label, in line with the text
            column rather than flush against the tile's left edge (where the
            desktop-only subContent below sits). */}
        {sub && (
          <span className={`md:hidden text-surface-500 text-sm ${rank ? 'pl-7' : ''}`}>{sub}</span>
        )}
      </span>
    </span>
  );
  ...
  const subContent = sub && (
    <span className="hidden md:inline text-surface-500 md:text-right text-sm shrink-0">{sub}</span>
  );
